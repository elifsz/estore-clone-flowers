...
function getCategory() {
  fetch(uriCategory)
    ...
    .then((data) => displayItemsforCategory(data))
    .catch((error) => console.error('Unable to get category.', error))
}

function displayItemsforCategory(data) {
  categories = data
}
...
function getflowerItems() {
  fetch(uriFlowers)
    .then((response) => response.json())
    .then((data) => displayFlowerItems(data))
    .catch((error) => console.error('Unable to get flowers.', error))
}

function displayFlowerItems(data) {
  getCategory()
  ...
  data.forEach((item) => {
    ...
    editButton.addEventListener(
      'click',
      function () {
        displayFlowerEditForm(item.flowerId)
      },
      false,
    )
    ...
    deleteButton.addEventListener(
      'click',
      function () {
        displayDeleteFlowerForm(item.flowerId)
      },
      false,
    )
...
function displayDeleteFlowerForm(id) {
  deleteFlowerId = id
...
function displayFlowerEditForm(id) {
  const item = flowers.find((item) => item.flowerId === id)
  editFlowerId = id
